...
class Channel extends Component {
    ...
    checkIfChannelOnline(channel) {
        const url = 'https://api.twitch.tv/kraken/streams/' + channel + '?client_id=qf8via2w923otpjjcjo1jeg6slulq4';
        return fetch(url).then((response) => response.json());
    }
    pullOfflineChannelData(channel) {
        const url = 'https://api.twitch.tv/kraken/channels/' + channel + '?client_id=qf8via2w923otpjjcjo1jeg6slulq4';
        return fetch(url).then((response) => response.json());
    }
    render() {
        if (!this.state.data) {
